...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      className={['btn', `btn--${resolvedVariant}`, `btn--${size}`, isDisabled ? 'btn--disabled' : '', isLoading ? 'btn--loading' : '', className || ''].filter(Boolean).join(' ')}
    ...
    >
      ...
      {iconRight && !isLoading && <span className="btn__icon btn__icon--right" aria-hidden="true">{iconRight}</span>}
    </button>
  );
};
