...
export const AccordionItem: React.FC<AccordionItemProps> = ({
  id, title, children, defaultOpen = false, actions, isOpen: controlledOpen, onToggle,
}) => {
    ...
    <div className={['accordion-item', isOpen ? 'accordion-item--open' : ''].filter(Boolean).join(' ')}>
      <div className="accordion-item__header" id={headerId}>
        ...
        <div className="accordion-item__actions">
          {actions || (
            <button type="button" className="accordion-item__more-btn" aria-label={`More options for ${title}`}>
              <DotsIcon />
            </button>
          )}
        </div>
      ...
      {isOpen && (
        <div id={panelId} className="accordion-item__panel" role="region" aria-labelledby={headerId}>
...
export const Accordion: React.FC<AccordionProps> = ({ items, allowMultiple = true, className }) => {
  ...
  return (
    <div className={['accordion', className || ''].filter(Boolean).join(' ')}>
      {items.map(item => (
        <AccordionItem key={item.id} id={item.id} title={item.title} isOpen={openIds.has(item.id)} onToggle={handleToggle} actions={item.actions}>
          {item.content}
        </AccordionItem>
      ))}
    </div>
